...
**API docs:** [traffisense-ai-2.onrender.com/docs](https://traffisense-ai-2.onrender.com/docs) — the bare API root (`/`) is a simple health check that returns `{"status": "ok", "service": "TraffiSense AI backend"}`; the actual functionality lives under `/docs` and the `/api/...` endpoints.
